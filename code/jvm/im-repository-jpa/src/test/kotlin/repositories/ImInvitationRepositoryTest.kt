...
open class ImInvitationRepositoryTest(
    ...
    @Test
    @Transactional
    open fun `should find invitation by token`() {
        val savedInvitation = imInvitationRepository.save(testInvitation1)
        val foundInvitation = imInvitationRepository.findByToken(savedInvitation.token)
    ...
    @Test
    @Transactional
    open fun `should return null for non-existent token`() {
        val foundInvitation = imInvitationRepository.findByToken(UUID.randomUUID())
